...
class Point:
    ...
    def run(self,):
        ...
        if 130 < self.distance(py.mouse.get_pos()) < 200:
            ...
            if x != 0 and (self.distance(py.mouse.get_pos()) <
                           110 or self.distance(py.mouse.get_pos()) > 130):
                self.position[0] += x / abs(x) * math.ceil(abs(x))
            if y != 0 and (self.distance(py.mouse.get_pos()) <
                           110 or self.distance(py.mouse.get_pos()) > 130):
    ...
    def distance(self, other):  # 求点距
        return math.sqrt(
            (self.position[0] - other[0])**2 + (self.position[1] - other[1])**2)
...
class Graph:
    ...
    def __init__(self, number):
        ...
        for i in range(number):
            self.pointlist.append(Point([random.randint(1, width), random.randint(
                1, height)], random.randint(1, 3), i / number * 2 * math.pi, 3))  # 根据number创建点个数
            ...
            if each.key == py.K_F11:
                ...
                if fullscreen:
                    ...
                    screen = py.display.set_mode(
                        (1920, 1080), py.FULLSCREEN | py.HWSURFACE)
                else:
                    mysize = width, height = 800, 600
                    screen = py.display.set_mode(mysize)
