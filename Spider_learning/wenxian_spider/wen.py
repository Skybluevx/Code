"""
    ...
    2020年4月20日之前：完成了对 CSDN 的爬取，初步尝试了对知网、知乎的爬取，但是失败，
                    目前还没有找到解决办法。而博客园的滑块验证目前已经快解决了，不过准确性还不高。
    2020年4月20日晚：完成了对博客园部分内容的解析。
"""
import time
import requests
...
class BlogSpider(object):
    ...
    def __init__(self):
        ...
        self.bg_img_name = "bg_img.png"
        self.slice_img_name = "slice_img.png"
    ...
    def refresh(self):
        refresh_button = self.driver.find_elements_by_class_name(
            "geetest_refresh_icon")[0]
        refresh_button.click()
        time.sleep(2)

    def yanzheng(self):
        self.get_img()
        value_1, value_2 = self.img_match_path(
            self.bg_img_name, self.slice_img_name)
    ...
    def generate_tracks(self, distance):
        ...
        out = distance - current
        back_tracks = [-3, -3, -2, -2, -2, -2, -2, -1, -1, -1, -1]
        back_tracks.append(out)
        # back_tracks.append(-20)
    ...
    def get_bg_img(self, scrennshot):
        ...
        self.deal_with_img(bg_img, self.bg_img_name)
        bg_img.save(self.bg_img_name)
    ...
    def get_slice_img(self, screenshot):
        ...
        self.deal_with_img(slice_img, self.slice_img_name)

    def deal_with_img(self, img, name):
        if name == self.bg_img_name:
            num = 4 / 5
        else:
            num = 1
        pixel = 220
    ...
    def get_infos(self, content):
        html = etree.HTML(content)
        infos = html.xpath('//div[@class="searchItem"]')
        infos_list = []
        for info in infos:
            info_dict = {}
            title = info.xpath('.//a[@target="_blank"]/text()')
            key_word = info.xpath('.//a[@target="_blank"]/strong/text()')
